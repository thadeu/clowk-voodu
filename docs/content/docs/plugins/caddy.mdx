...
### A service on another voodu host

On a host wired to others (see [Cross-VM networking](/docs/getting-started/cross-vm)), an `ingress` may name a service that runs on another host. The controller finds no local deployment, asks the mesh for `<service>.<scope>.voodu`, and when another host answers hands Caddy that name. Caddy resolves it on every dial, so a deploy on the other host needs nothing here.

```hcl
ingress "clowk" "api" {
  host    = "api.example.com"
  service = "api"
  port    = 8080   # required: the service's manifest is not on this host
}
```

Caddy is started with the mesh resolvers by the plugin install, so re-run `voodu plugins:install thadeu/voodu-caddy` once on a host migrated to a routed `voodu0`.

...
**Firewall.** The installer opens what the mesh needs when ufw is active:
UDP 51820, DNS on `wg0` and on the `voodu0` bridge, and forwarding from `wg0`
into the bridge. With ufw inactive it prints the rules instead. They are
available at any time, with the bridge's real name:

```bash
vd wire ufw -r vm-1          # print
sudo vd wire ufw enable      # apply, on the host
```
...
UDP and TCP. The two DNS rules above are what lets both in.
...
3. **Recreate the network**, on the host:
   ...
   docker network create --driver bridge \
     --subnet 10.91.221.0/24 --gateway 10.91.221.1 --ip-range 10.91.221.128/25 \

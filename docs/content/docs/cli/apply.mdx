...
Private registries authenticate through the `auths` entries a
[`registry`](/docs/manifests/registry) manifest writes, the same ones
the container-create path uses. ECR tokens expire every 12 hours — see
the [token-rotation cronjob](/docs/manifests/registry#token-rotation-via-cronjob-ecr).

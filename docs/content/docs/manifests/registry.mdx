...
Then name the helper for your registry host in the config voodu owns,
`$VOODU_ROOT/docker/config.json`:

```json
{
  "credHelpers": {
    "123456789012.dkr.ecr.sa-east-1.amazonaws.com": "ecr-login"
  }
}
```

Voodu preserves `credHelpers` verbatim across reconciles — it claims
only the `auths` section — so this coexists with `registry` blocks for
other hosts, and survives an apply that touches none.

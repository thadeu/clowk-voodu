...
This one only does something when a plugin is watching the workload's replicas — [`trafik`](https://github.com/thadeu/voodu-trafik), for instance, which counts live connections and reports when the last one closes. Without such a plugin, `timeout` is inert and `grace` is the whole story.
